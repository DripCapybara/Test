...
if victim then
   if workspace:FindFirstChild(victim.Name) then
      if part:IsDescendantOf(victim.Character) then
          if part.Name == "Torso" or part.Name == "UpperTorso" then
             victim.Character.HumanoidRootPart.CFrame=victim.Character.HumanoidRootPart.CFrame+AMNT
          end
      end
   end
end
...
-- Properties
if game.Players.LocalPlayer.PlayerGui:FindFirstChild("nil") then
game.Players.LocalPlayer.PlayerGui:FindFirstChild("nil"):Destroy()
end
ScreenGui.Name = "nil"
...
if not workspace:FindFirstChild(tostring(TextBox.Text)) then
return
end
...
light2.Brightness = 0.2
light2.Color = Color3.fromRGB(255,0,0)
light2.Range = 20
light2.Enabled = false
light2.Parent = fistmesh

-- 8 - Evil Doge Head
local doge = Instance.new("Part")
doge.CFrame = CFrame.new(Vector3.new(13.4999914, 4.49999952, 143.299988)) * CFrame.Angles(-1.570796251297, 0.84960347414017, -1.5707963705063)
doge.FormFactor = Enum.FormFactor.Symmetric
doge.TopSurface = Enum.SurfaceType.Weld
doge.BottomSurface = Enum.SurfaceType.Weld
doge.Material = Enum.Material.Neon
doge.Size = Vector3.new(8, 9, 2)
doge.Transparency = 0
doge.Anchored = true
doge.BrickColor = BrickColor.new("Really black")
doge.Friction = 0.50000001192093
doge.Shape = Enum.PartType.Block
doge.Name = "evil doge"
...
VAL=plr.Character.Torso.CFrame
	...
	elseif plr.Character:FindFirstChild('UpperTorso') and plr.Character:FindFirstChild('HumanoidRootPart') then
		...
		VAL=plr.Character.UpperTorso.CFrame
	end
	...
	sound.Parent = thing
	sound.SoundId = "rbxassetid://743947235"
	sound.Volume = 1
	sound.Looped = true
	sound:Play()
	local tweened = false
	for i=1,120 do
		if thing then
		if i==110 then
		if tweened == false then tweened = true
		br=game.TweenService:Create(plr.Character.Torso,TweenInfo.new(1,Enum.EasingStyle.Cubic),{CFrame=VAL})
		br:Play()
		end
		end
		if i==110 then
		plr.Character:FindFirstChildOfClass("Humanoid").Animator:Destroy()
		plr.Character.Torso.Anchored = true
		end
   ...
   for i=1,50 do
	...
	if plr.Character:FindFirstChild('Torso') then
		...
		Instance.new("Animator",plr.Character:FindFirstChildOfClass("Humanoid"))
	end
	...
	for i=1,200 do
	if plr.Character:FindFirstChildOfClass('Humanoid') then
		plr.Character:FindFirstChildOfClass('Humanoid'):TakeDamage(0.22)
		print(plr.Character:FindFirstChildOfClass('Humanoid').Health)
	end
	MoveBy(obj1,Vector3.new(0,-1,0))
	...
	if plr.Character:FindFirstChildOfClass('Humanoid') then
	local crush = Instance.new("Sound",plr.Character.Head)
	crush.SoundId = "rbxassetid://3782301710"
	crush.Volume = 3
	crush:Play()
	game.Debris:AddItem(crush,7)
